...
class Publication:
    ...
    def add_meta_description_creator_040(self):
        ...
        if len(self.body.get("_publication_orgs", [])) > 1:
            other_orgs = [org for org in self.body.get("_publication_orgs", []) if org != self.meta_assigner_label]
            if other_orgs:
        ...
        #if self.bucket_info.candidates and len(self.bucket_info.candidates) > 0:
        #    candidate_meta_assigner_label = self.bucket_info.candidates[0].meta_assigner_label
        #
        #    meta['descriptionUpgrader'] = {
        #        '@id': 'https://libris.kb.se/library/' + str(candidate_meta_assigner_label),
        #        '@type': 'Library',
    ...
    def add_publication_008(self):
        ...
        if body.get('publication', []):
            for publication in [p for p in body.get('publication') if p.get('@type') == 'Publication']:
    ...
    def add_meta_duplicate_publications_009(self):
        ...
        for duplicated_publication in [p for p in self.body.get("_publication_ids", []) if p != self.id]:
    ...
    def add_instance_of_contribution_100_and_700_710(self):
        ...
        if body.get('instanceOf'):
            ...
            contributions = [c for c in contributions if c.get('agent') and c.get('agent').get('@type') == 'Person']
            ...
            for c in contributions:
                ...
                if c.get('agent') and c.get('hasAffiliation'):
                    ...
                    if c.get('agent', {}).get('termsOfAddress'):
                        c.get('agent')['marc:titlesAndOtherWordsAssociatedWithAName'] \
                            = c.get('agent').get('termsOfAddress')

                    for id_by in c.get('agent', {}).get("identifiedBy", []):
                        if id_by.get('@type') == 'Local' and id_by.get('source', {}).get('code'):
                            c.get('agent')['marc:uri'] = '(Swepub:' + str(
                                id_by.get('source', {}).get('code')) + ')' + str(id_by.get('value'))

            _move_tmp_global_organizational_contribution_to_instance_of_contribution_for_marc_field_710(body)
    ...
    def add_part_of_773(self):
        ...
        if body.get('partOf'):
            ...
            for is_part in before_convert:
                ...
                if is_part.get('hasTitle') and len(is_part.get('hasTitle')) > 0 \
                        and not is_part.get('genreForm') and is_part['@type'] != 'Dataset':
                    ...
                    if issue_number:
                        ...
                        marc_enumeration_and_first_page = marc_enumeration_and_first_page + ':' + issue_number
        ...
        if body.get('electronicLocator'):
            ...
            for e_l in electronic_locators:
                ...
                if e_l.get('locator'):
                    ...
                    e_l['uri'] = [e_l.get('uri', '')]
                if e_l.get('hasNote'):
                    e_l['cataloguersNote'] = [note.get('label') for note in e_l.pop('hasNote', []) if note.get('label')]
                if e_l.get('label'):
...
def _set_global_organizational_contribution_for_marc_field_710(body, affiliation):
    if body.get('tmp_global_organizational_contribution'):
        ...
        if not is_part_of and agent_name != affiliation.get("name"):
            ...
            if marc_subordinate_unit not in marc_subordinate_units and marc_subordinate_unit != is_part_of.get('name'):
...
def _move_tmp_global_organizational_contribution_to_instance_of_contribution_for_marc_field_710(body):
    if body.get('tmp_global_organizational_contribution'):
...
def _set_broader_term_component_list(subject, broader):
    ...
    if not broader.get('broader'):
        ...
        topic_sub_division = {
            "@type": "TopicSubdivision",
            "prefLabel": broader.get('prefLabel')
        }
        if topic_sub_division not in subject.get('termComponentList'):
