...
issn_regex = re.compile(
    r'(?:(?<!(\d[^0-9-])))' +  # Check that not preceded by digit or and not number or hyphen
    r'(?:(?<![0-9-]))' +       # Check that not preceded by digit
    r'([0-9]{4})' +            # Four consecutive numbers
    r'(-?)' +                  # Accept a correct delimiter if one exists
    r'[^0-9]*' +               # Any number of non-digit signs as delimiter
    r'([0-9]{3})' +            # Three consecutive numbers
    r'[^0-9]??' +              # optional non-numerical sign before the control number
    r'([0-9xX])' +             # Control number
    r'(?:(?![0-9-]))' +        # Make sure the code isn't followed by a number or hyphen
    r'(?:(?![ ]\d))'           # Check that not followed by non digit or hyphen and digit
)
